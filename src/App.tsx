...
function App() {
    ...
    useEffect(() => {
        ...
        const loadNotifications = async () => {
            try {
                const response = await fetch(`${API_URL}/api/notifications?accountId=${account.id}`);
                if (response.ok) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (ringingCalls.length > 0 && currentPage !== 'meeting') {
                        ...
                        if (!incomingCall || incomingCall.id !== newCall.id) {
                            setIncomingCall(newCall);
                            callRingtone.play().catch(err => console.error('Failed to play ringtone:', err));
                        }
                    } else if (incomingCall && ringingCalls.length === 0) {
                        callRingtone.pause();
